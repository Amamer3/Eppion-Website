...
const useTypewriter = (text: string, delay: number = 100) => {
  ...
  useEffect(() => {
    ...
    const animationDuration = Math.floor(Math.random() * (10 - 5 + 1) + 5) * 60 * 1000;
...
const Hero = () => {
  ...
  const { displayText: title1, isTypingComplete: title1Complete } = useTypewriter('Digital Network Globe . . .', 200);
  const { displayText: title2, isTypingComplete: title2Complete } = useTypewriter('Empowering Innovation in Technology and Marketing', 100);
